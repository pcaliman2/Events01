...
import 'package:owa_flutter/widgets/owa_intro_gate.dart';
...
const Duration _signupPopoutDelay = Duration(seconds: 14);
...
class _HeroSectionState extends State<HeroSection>
  ...
  // 3) Timer field
  Timer? _popoutTimer;
  bool _heroAnimationsStarted = false;
  ...
  void didChangeDependencies() {
    ...
    final revealProgress = OWAIntroRevealScope.progressOf(context);
    if (!_heroAnimationsStarted && revealProgress >= 0.68) {
      _startHeroAnimations();
    }
  }

  void _startHeroAnimations() {
    if (_heroAnimationsStarted) return;
  ...
  Widget build(BuildContext context) {
    final introReveal = Curves.easeInOutCubic.transform(
      ((OWAIntroRevealScope.progressOf(context) - 0.08) / 0.92).clamp(0.0, 1.0),
    );
    final heroOpacity = lerpDouble(0.0, 1.0, introReveal)!;
    final heroScale = lerpDouble(1.03, 1.0, introReveal)!;
    final heroTranslateY = lerpDouble(34, 0, introReveal)!;
    final heroBlur = lerpDouble(12, 0, introReveal)!;

    return Opacity(
      opacity: heroOpacity,
      child: ImageFiltered(
        imageFilter: ImageFilter.blur(sigmaX: heroBlur, sigmaY: heroBlur),
  ...
  Widget _buildNavItem(String text) {
    return GestureDetector(
      ...
      child: PreciseAnimatedNavItem(text: text),
    );
  }
